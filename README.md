...
9. **damagetakenperminute**: Damage taken per minute, indicating the amount of damage a player have taken from attacks of others in one minute.

10. **gamelength**: How long did a player played in a single eSport match, marked in seconds.

11. **wpm**: Ward per minute, indicating the amount of wards a player placed in one minute.

12. **vspm**: Vision score per minute, a indicator of a player's contribution to their team's control of the map by either placing wards or removing enemy wards.

13. **dpm**: Damage per minute, indicating the amount of damage a player made in one minute.

14. **result**: The result of a single game in one match, either win(Denoted by 1) or lose(Denoted by 0). 

15. **monsterkills**: The number of monsters killed in one game, including dragons/baron. 
...
## NMAR Analysis
NMAR means that there is a good reason why the missingness depends on the values of a column themselves. We don't believe there are any column in our dataset that is NMAR because we believed that the missingness of the "teamname" column depends on other columns in this dataset.

We would like to obtain additional information about possible correlation between **datacompleteness** and **position** column and missingness in teamname column, as the **"teamname"** column is one of the column in this dataset that contains missing values. 

* In League of Legends, many variables can influence the missingness of the "teamname" column in the dataset including the team roster changes, the type of professional competitions like regional playoffs versus world championship, incomplete games with unexpected technical difficulties, etc.
...
<iframe src="assets/fairness2.html" width=800 height=600 frameBorder=0></iframe>
